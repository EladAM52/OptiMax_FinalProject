                    ...
                    <div key={index} className="shift-table-row">
                        ...
                        {weekDates.map((date, index) => (
                            <div key={index} className="shift-table-cell">
                                <select
                                    value={arrangements.find(arr => arr.date === date)?.[shiftType.key] || ''}
                                    onChange={(e) => handleShiftChange(date, shiftType.key, e.target.value)}
                                >
                                    <option value="">בחר עובד</option>
                                    {(availableEmployees[date]?.[shiftType.key] || []).map(emp => (
                                        <option key={`${date}-${shiftType.key}-${emp._id}`} value={emp._id}>{emp.FirstName} {emp.LastName}</option>
                                    ))}
                                </select>
                            </div>
                        ))}
